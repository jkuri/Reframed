  ...
  AVCaptureAudioDataOutputSampleBufferDelegate, @unchecked Sendable
{
  ...
  private let logger = Logger(label: "eu.jankuri.frame.device-capture")
  private var isPaused = false
  private let verifyQueue = DispatchQueue(label: "eu.jankuri.frame.device-verify", qos: .userInteractive)
  private let audioQueue = DispatchQueue(label: "eu.jankuri.frame.device-audio", qos: .userInteractive)
  ...
  func startAndVerify(deviceId: String) async throws -> VerifiedDevice {
    ...
    let startQueue = DispatchQueue(label: "eu.jankuri.frame.device-start")
    startQueue.async {
